...
import {
  ...
  TrendingUp, TrendingDown, Minus, Loader2, RefreshCw, Settings, Target, Leaf, FileStack, Flame
} from 'lucide-react';
...
// 콘텐츠 카드 컴포넌트
// ============================================================
function ContentCard({ item, coreKeys, subKeys, metricsMap, onSave, onDelete, showSalesImpact, showScoreTracking, retroBadge, onSyncInsight, avgMetrics, showThumbnailUpload }) {
  ...
  if (editing) {
    return (
      <div style={{ background: C.card, border: `1px solid ${C.accent}`, borderRadius: 14, padding: 16, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-wrap gap-3 mb-3">
          {coreKeys.map((k) => (
             <NumberField key={k} label={k === 'profileActivity' ? '프로필 활동 [수동 기입]' : metricsMap[k]?.label} value={draft[k]} onChange={set(k)} width={120} />
          ))}
        </div>
              ...
              <thead>
                <tr style={{ borderBottom: `2px solid ${C.border}`, background: C.panel }}>
                  {['월', '매출', '유입', '도달수', '참여수'].map((h) => (
                    <th key={h} style={{ textAlign: h === '월' ? 'left' : 'right', padding: '6px 10px', fontWeight: 700, color: C.sub }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
...
function CountryView({
  ...
  const primaryTrendNorm = trendData.map((d) => {
    ...
    PRIMARY_KEYS.forEach((k) => {
      entry[k] = Math.round((d[k] / primaryBase[k]) * 100);
      entry[`_raw_${k}`] = d[k];
    });
    return entry;
  });
  ...
  const updateAllItem = (item) => {
    ...
    onAllContentsChange(countryKey, selectedWeek, list);
  };
    ...
    <div>
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: C.sub }}>첫 주 기준(=100) 정규화 적용 · 지표 간 볼륨 편차를 제거하여 성장 속도를 비교합니다.</div>
          </div>
          <button onClick={() => setShowPrimaryTable((v) => !v)} className="flex items-center gap-1.5"
            style={{ fontSize: 12, fontWeight: 700, padding: '6px 12px', borderRadius: 8, border: `1px solid ${C.border}`, background: showPrimaryTable ? C.ink : '#fff', color: showPrimaryTable ? '#fff' : C.sub, cursor: 'pointer' }}>
            {showPrimaryTable ? <ChevronUp size={13} /> : <ChevronDown size={13} />} 데이터표
          </button>
        </div>
              ...
              <thead>
                <tr style={{ borderBottom: `2px solid ${C.border}`, background: C.panel }}>
                  {['주차', '매출', '유입', '도달수', '참여수'].map((h) => (
                    <th key={h} style={{ textAlign: h === '주차' ? 'left' : 'right', padding: '6px 10px', fontWeight: 700, color: C.sub }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {topContent.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onSave={updateAllItem} onDelete={deleteAllItem} showSalesImpact showScoreTracking onSyncInsight={(i) => handleSyncContent(i, '상위')} avgMetrics={avgMetrics} metricKey="reachAndEngagement" />)}
        </div>
      </div>
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {bottomContent.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onSave={updateAllItem} onDelete={deleteAllItem} showSalesImpact showScoreTracking onSyncInsight={(i) => handleSyncContent(i, '하위')} avgMetrics={avgMetrics} metricKey="reachAndEngagement" />)}
        </div>
      </div>

      {/* 🔥 [신설] 게시 전체 콘텐츠 리스트 토글 (평균 수비형 콘텐츠 관리용) */}
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        {showAllList && (
          <div className="flex flex-col gap-2.5 mt-4">
            ...
            {weekItems.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onSave={updateAllItem} onDelete={deleteAllItem} showSalesImpact showScoreTracking onSyncInsight={(i) => handleSyncContent(i, '전체')} avgMetrics={avgMetrics} metricKey="reachAndEngagement" />)}
          </div>
        )}
      </div>
      ...
      <div className="mb-4" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
          <div>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: C.sub }}>[참고] {country.label} 월간 누적 현황 차트</h3>
          </div>
...
function FeedView({ weekMeta, selectedWeek, displayWeeks, feedContents, accountMetrics, onFeedContentsChange, onSyncContent }) {
  ...
  // 해당 주차 피드 자체 데이터 지표 총합 계산기
  const getFeedTotals = useCallback((weekKey) => {
    const list = countryContents[weekKey] || [];
    return {
  ...
  const calcFeedDelta = (key) => {
    if (!prevTotals || prevTotals[key] === 0) return null;
    return ((currentTotals[key] - prevTotals[key]) / prevTotals[key]) * 100;
  };

  // 최근 7주 주간 추이 차트 데이터 세팅
  const weeklyTrendData = displayWeeks.map(wk => ({ week: wk, ...getFeedTotals(wk) }));
  const wBase = {
    reach: weeklyTrendData.find(d => d.reach > 0)?.reach || 1,
    engagement: weeklyTrendData.find(d => d.engagement > 0)?.engagement || 1,
    saves: weeklyTrendData.find(d => d.saves > 0)?.saves || 1,
    shares: weeklyTrendData.find(d => d.shares > 0)?.shares || 1,
    profileActivity: weeklyTrendData.find(d => d.profileActivity > 0)?.profileActivity || 1,
  };
  const weeklyTrendNorm = weeklyTrendData.map(d => ({
    week: d.week,
    reach: Math.round((d.reach / wBase.reach) * 100),
    engagement: Math.round((d.engagement / wBase.engagement) * 100),
    saves: Math.round((d.saves / wBase.saves) * 100),
    shares: Math.round((d.shares / wBase.shares) * 100),
    profileActivity: Math.round((d.profileActivity / wBase.profileActivity) * 100),
    _raw_reach: d.reach,
    _raw_engagement: d.engagement,
    _raw_saves: d.saves,
  ...
  // 연간 누적 월간 추이 차트 세팅 (저장수, 공유수, 프로필 활동)
  const latestMonthStr = weekMeta[weekMeta.length - 1]?.month || '2026-06';
  const currentYear = latestMonthStr.split('-')[0];
  const latestMonthNum = parseInt(latestMonthStr.split('-')[1], 10);
  const monthList = Array.from({length: latestMonthNum}, (_, i) => `${currentYear}-${String(i+1).padStart(2, '0')}`);
  ...
  const deleteContent = (id) => {
    const list = weekContents.filter((c) => c.id !== id);
    onFeedContentsChange(selectedCountry, selectedWeek, list);
  };
  const addContent = () => {
  ...
  return (
    <div>
      ...
      {/* ⭐️ [복구 완료] 피드 주간 성과 추이 차트 (최근 7주 수평 매칭) */}
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
          <div>
            <h3 style={{ fontSize: 15, fontWeight: 800, margin: '0 0 2px' }}>피드 주간 핵심지표 추이 비교 (최근 7주)</h3>
            <div style={{ fontSize: 12, color: C.sub }}>첫 주 기준(=100) 정규화 적용 · 도달/참여 및 수동기입 지표 추적용</div>
          </div>
          <button onClick={() => setShowWeeklyChartTable(!showWeeklyChartTable)} className="flex items-center gap-1.5" style={{ fontSize: 12, fontWeight: 700, padding: '6px 12px', border: `1px solid ${C.border}`, background: showWeeklyChartTable ? C.ink : '#fff', color: showWeeklyChartTable ? '#fff' : C.sub, cursor: 'pointer' }}>데이터표</button>
        </div>
        <div style={{ width: '100%', height: 230 }}>
          <ResponsiveContainer>
            <LineChart data={weeklyTrendNorm} margin={{ top: 15, right: 10, left: -10, bottom: 0 }}>
              <CartesianGrid stroke={C.border} vertical={false} />
              <XAxis dataKey="week" tick={{ fontSize: 12 }} axisLine={{ stroke: C.border }} tickLine={false} />
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW, marginBottom: 24 }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
          ...
          <button onClick={() => setShowFollowerCompareTable(!showFollowerTable)} className="flex items-center gap-1.5" style={{ fontSize: 12, fontWeight: 700, padding: '6px 12px', border: `1px solid ${C.border}`, background: showFollowerTable ? C.ink : '#fff', color: showFollowerTable ? '#fff' : C.sub, cursor: 'pointer' }}>데이터표</button>
        </div>
        <div style={{ width: '100%', height: 200 }}>
          <ResponsiveContainer>
      ...
      <button onClick={addContent} className="flex items-center gap-1 mb-4" style={{ fontSize: 13, fontWeight: 700, padding: '7px 12px', border: 'none', background: C.ink, color: '#fff' }}><Plus size={14} /> 피드 콘텐츠 추가</button>
      ...
      {/* 📉 피드 월간 누적 추이 차트 (하단으로 내림 - 저장, 공유, 프로필 활동) */}
      <div className="mb-4" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
          <div>
            <h3 style={{ fontSize: 14, fontWeight: 800, color: C.sub }}>[참고] 피드 월간 지표 추이 (저장·공유·프로필 활동)</h3>
          </div>
          <button onClick={() => setShowMonthlyTable(!showMonthlyTable)} className="flex items-center gap-1" style={{ fontSize: 11, border: `1px solid ${C.border}`, background: '#fff', padding: '4px 8px', borderRadius: 6, cursor: 'pointer' }}>데이터표</button>
        </div>
        <div style={{ width: '100%', height: 160 }}>
...
export default function App() {
  ...
  // 국가별 대시보드 저장 바인딩용 메인 래퍼 함수 ⭐️
  const handleAllContentsChange = useCallback((country, week, newList) => {
    setAllContents(prev => ({
      ...prev,
      [country]: { ...prev[country], [week]: newList }
    }));
  }, []);

  // 피드 대시보드 저장 바인딩용 메인 래퍼 함수 ⭐️
      ...
      <div className="max-w-[1400px] mx-auto w-full" style={{ padding: '24px 20px' }}>
        ...
        {(view === 'KR' || view === 'US') && (
          <CountryView
            ...
            allContents={allContents} onAllContentsChange={handleAllContentsChange} onSyncContent={syncToGAS}
          />
        )}
        {view === 'feed' && (
          <FeedView weekMeta={weekMeta} selectedWeek={selectedWeek} displayWeeks={displayWeeks} feedContents={feedContents} accountMetrics={accountMetrics} onFeedContentsChange={handleFeedContentsChange} onSyncContent={syncToGAS} />
        )}
