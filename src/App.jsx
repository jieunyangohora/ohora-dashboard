...
// ✅ 누락되었던 모든 아이콘 라이브러리들을 상단에 완벽하게 바인딩 완료했습니다.
import {
...
function MetricPill({ metricsMap, mkey, value, big }) {
  ...
  return (
    <div className="flex items-center gap-1.5" style={{ color: C.ink }}>
      ...
      <MyIcon size={big ? 15 : 13} color={C.sub} strokeWidth={2} />
...
function DeltaTag({ value }) {
  ...
  const MyIcon = flat ? Minus : up ? TrendingUp : TrendingDown;
  return (
    <span className="flex items-center gap-0.5" style={{ color, fontSize: 12, fontWeight: 700 }}>
      <MyIcon size={13} strokeWidth={2.5} />{pct(value)}
    </span>
  );
}
...
function SyncBadge({ status }) {
  ...
  const map = {
    syncing: { icon: RefreshCw, text: '동기화 중...', color: C.sub },
    ok:      { icon: CheckCircle, text: '시트 저장됨', color: C.mint },
    error:   { icon: AlertCircle, text: '연동 실패', color: C.accent },
  };
  const m = map[status];
  if (!m) return null;
  const MyIcon = m.icon;
  return (
    <span className="flex items-center gap-1" style={{ fontSize: 11, fontWeight: 700, color: m.color, padding: '4px 10px', borderRadius: 999, border: `1px solid ${m.color}22`, background: `${m.color}11` }}>
      <MyIcon size={12} className={status === 'syncing' ? "animate-spin" : ""} />{m.text}
    </span>
  );
}

// ✅ 옵셔널 체이닝 안전장치를 강화하여 차트 런타임 크래시를 방지하는 툴팁
      ...
      <div style={{ background: C.card, border: `1px solid ${C.accent}`, borderRadius: 14, padding: 16, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          <div style={{ flex: '1 1 240px' }}>
            <TextAreaField label="🤔 가설" value={draft.hypothesis} onChange={set('hypothesis')} placeholder="발행 전 가설과 실험 요소를 적어주세요." />
          </div>
          <div style={{ flex: '1 1 240px' }}>
            <TextAreaField label="📝 분석 & 추후 방안" value={draft.analysis} onChange={set('analysis')} placeholder="결과 분석과 다음 액션 플랜을 적어주세요." />
          </div>
        </div>
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2">
            {[{ key: '', label: '미분류' }, ...PRODUCT_CATS].map((p) => (
              <button key={p.key} onClick={() => setDraft((d) => ({ ...d, productCategory: p.key }))}
                style={{ fontSize: 12, fontWeight: 700, padding: '5px 14px', borderRadius: 999, border: `1.5px solid ${draft.productCategory === p.key ? p.color : C.border}`, background: draft.productCategory === p.key ? `${p.color}15` : '#fff', color: draft.productCategory === p.key ? p.color : C.sub, cursor: 'pointer' }}>
        ...
        {showSalesImpact && (
          <div className="mb-4">
            <TextAreaField label="매출전환효과" value={draft.salesImpact || ''} onChange={set('salesImpact')}
              placeholder="매출전환 인과과정 효과를 적어주세요." rows={2} />
          </div>
        )}
...
function CountryView({
  ...
  return (
    <div>
      ...
      {/* ⭐️ 주차별 성과 추이 차트 (최근 7주 메인 상단 배치 완료) */}
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-col gap-2.5 mt-2">
          ...
          {topContent.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onDelete={deleteAllItem} onSave={updateAllItem} avgMetrics={avgMetrics} showSalesImpact onSyncInsight={(i) => handleSyncContent(i, '상위')} />)}
        </div>
      </div>
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        <div className="flex flex-col gap-2.5 mt-2">
          ...
          {bottomContent.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onDelete={deleteAllItem} onSave={updateAllItem} avgMetrics={avgMetrics} showSalesImpact onSyncInsight={(i) => handleSyncContent(i, '하위')} />)}
        </div>
      </div>
      ...
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        ...
        {showAllList && (
          <div className="flex flex-col gap-2.5 mt-4">
            {weekItems.map((item) => <ContentCard key={item.id} item={item} coreKeys={CONTENT_CORE} subKeys={CONTENT_SUB} metricsMap={CONTENT_METRICS} onDelete={deleteAllItem} onSave={updateAllItem} avgMetrics={avgMetrics} showSalesImpact onSyncInsight={(i) => handleSyncContent(i, '전체')} />)}
          </div>
        )}
      </div>
...
function FeedView({ weekMeta, selectedWeek, displayWeeks, feedContents, accountMetrics, onFeedContentsChange, onSyncContent }) {
  ...
  return (
    <div>
      ...
      {/* ⭐️ 피드 주간 핵심지표 추이 비교 (최근 7주 상단 메인 안착) */}
      <div className="mb-6" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW }}>
        <div className="flex items-center justify-between flex-wrap gap-2" style={{ marginBottom: 4 }}>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, padding: 18, boxShadow: SHADOW, marginBottom: 24 }}>
        <div className="flex items-center gap-2 mb-4"><h3 style={{ fontSize: 15, fontWeight: 800, color: C.ink, margin: 0 }}>주차별 피드 발생 팔로우 · 계정 전체 신규 팔로우 증감률</h3><InfoTip text="피드 콘텐츠만의 합계가 아닌 계정 전체 신규 팔로워 증감률 현황입니다." /></div>
        <div style={{ width: '100%', height: 200 }}>
          <ResponsiveContainer>
      ...
      <div className="flex flex-col gap-2.5 mb-8">
        ...
        {weekContents.map((item) => <ContentCard key={item.id} item={item} coreKeys={FEED_CORE} subKeys={FEED_SUB} metricsMap={FEED_METRICS} onDelete={deleteContent} onSave={updateContent} avgMetrics={null} onSyncInsight={handleSyncFeedContent} showThumbnailUpload />)}
      </div>
